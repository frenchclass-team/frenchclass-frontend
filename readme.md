...
## 0.4. Git 协作规范 (Sourcetree 版)

### 0.4.1 创建功能分支
* 点击顶部工具栏的 **“分支 (Branch)”** 图标。
* 在弹出的窗口中输入新分支名称。命名规范：`ui/xxx` 或 `feature/xxx`。（`main` 分支受保护，仅存放审核通过的代码，禁止直接在 `main` 上修改。）
* 点击 **“创建分支 (Create Branch)”**。

### 0.4.2 提交代码 (Commit)
* 写完代码后回到 Sourcetree，在中下部点击“未暂存文件”后面的加号使其变为“已暂存文件”。
* 点击左上角的 **“提交”** 按钮，在底部文本框输入提交摘要，格式参考：`feat: 完成登录页面布局`。
* 点击右下角的 **“提交 (Commit)”** 按钮。

### 0.4.3 推送与创建 PR
* 点击顶部工具栏的 **“推送 (Push)”** 图标，确保勾选了你当前的功能分支。
* 推送成功后，Sourcetree 不会自动创建 PR，你需要**手动打开浏览器**进入项目的 GitHub 页面。
* 在 GitHub 页面点击 **“Compare & pull request”**，确认对比方向为：`base: main` ← `compare: ui/xxx`。
* 在右侧 **Reviewers** 处勾选 `june-sf` 并通知其进行审核合并。

### 0.4.4 同步最新代码
* 当 PR 被合并后，在左侧侧边栏双击 **`main`** 分支进行切换。
* 点击顶部工具栏的 **“抓取 (Fetch)”**，查看云端更新。
* 点击 **“拉取 (Pull)”** 图标，将最新的代码同步到你本地的 `main` 分支。
